function [pose] = lbl2d_depth(ranges,Beacon,depth,blFlag)
...
% Beacon - 2x3 matrix of beacon locations (each row is a beacon)
% depth  - scalar depth
% blFlag - forces the ambiguity in the solutio
%           default = 1 - clockwise, set to -1 for counterclockwise
% OUT
% pose - 3d pose (or -1 if the geometry is bad)
%
% HISTORY
...
for ii = 1:length(ranges)
    dz = depth - Beacon(ii,3);
    ranges(ii) = sqrt(ranges3d(ii)^2-dz^2);
end
